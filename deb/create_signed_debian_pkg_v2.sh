...
# example : create_signed_debian_pkg.sh sddm@all "Arijit Bhowmick" sys41x4@212D2254446D6161AA880823141531775A06F762
...
cd ../ && \
## Sign .deb package
dpkg-sig -e \"${maintainer}\" --sign builder -k "${gpg_key}" ./*.deb && \
## Sign *.changes and *.buildinfo file
debsign -e\"${maintainer}\" "-k${gpg_key}" ./*.changes \
" && cd ../ && pwd && \
#exit
...
echo "BUILDS are available at BUILD_${pkg_dir_name}"
